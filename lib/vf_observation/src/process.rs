...
impl Updateable<UpdateRequest> for Entry {
    fn update_with(&self, e: &UpdateRequest) -> Entry {
        Entry {
            ...
            before: if e.before.is_some() { e.before.to_owned().into() } else { self.before.to_owned() },
            after: if e.after.is_some() { e.after.to_owned().into() } else { self.after.to_owned() },
            classified_as: if e.classified_as.is_some() { e.classified_as.to_owned().into() } else { self.classified_as.to_owned() },
            based_on: if e.based_on.is_some() { e.based_on.to_owned().into() } else { self.based_on.to_owned() },
            planned_within: if e.planned_within.is_some() { e.planned_within.to_owned().into() } else { self.planned_within.to_owned() },
