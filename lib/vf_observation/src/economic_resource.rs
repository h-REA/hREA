...
pub struct UpdateRequest {
    ...
    contained_in: MaybeUndefined<ResourceAddress>,
    #[serde(default)]
    stage: MaybeUndefined<ProcessSpecificationAddress>,
    #[serde(default)]
    current_location: MaybeUndefined<LocationAddress>,
    #[serde(default)]
    note: MaybeUndefined<String>,
...
impl From<CreateRequest> for Entry {
    fn from(e: CreateRequest) -> Entry {
        Entry {
            ...
            classified_as: if e.classified_as == MaybeUndefined::Undefined { None } else { e.classified_as.to_owned().to_option() },
            tracking_identifier: if e.tracking_identifier == MaybeUndefined::Undefined { None } else { e.tracking_identifier.to_owned().to_option() },
            lot: if e.lot == MaybeUndefined::Undefined { None } else { e.lot.to_owned().to_option() },
            image: if e.image == MaybeUndefined::Undefined { None } else { e.image.to_owned().to_option() },
            accounting_quantity: if e.accounting_quantity == MaybeUndefined::Undefined { None } else { e.accounting_quantity.to_owned().to_option() },
            onhand_quantity: if e.onhand_quantity == MaybeUndefined::Undefined { None } else { e.onhand_quantity.to_owned().to_option() },
            unit_of_effort: None,   // :TODO:
            stage: None, // :TODO: check this
            state: None, // :TODO: check this if e.state == MaybeUndefined::Undefined { None } else { e.state.clone().into() },
            current_location: if e.current_location == MaybeUndefined::Undefined { None } else { e.current_location.to_owned().to_option() },
            note: if e.note == MaybeUndefined::Undefined { None } else { e.note.clone().into() },
        }
    }
}
...
impl Updateable<UpdateRequest> for Entry {
    fn update_with(&self, e: &UpdateRequest) -> Entry {
        Entry {
            ...
            stage: if e.stage == MaybeUndefined::Undefined { self.stage.to_owned() } else { e.stage.to_owned().to_option() },
            state: self.state.to_owned(), // :TODO: check this if e.state == MaybeUndefined::Undefined { self.state } else { e.state.to_option() },
...
impl Updateable<EventCreateRequest> for Entry {
    fn update_with(&self, e: &EventCreateRequest) -> Entry {
        Entry {
            ...
            accounting_quantity: if let Some(qty) = self.accounting_quantity.to_owned() {
                if let MaybeUndefined::Some(event_resource_qty) = e.resource_quantity.to_owned() {
                    match get_builtin_action(e.action.as_ref()) {
                        Some(action) => {
                            match action.resource_effect {
                                ...
                                _ => self.accounting_quantity.to_owned(),
                            }
                        },
                        None => self.accounting_quantity.to_owned(),
                    }
                } else { self.accounting_quantity.to_owned() }
            } else { None },
            onhand_quantity: self.onhand_quantity.to_owned(),  // :TODO: how is this affected?
