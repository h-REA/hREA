...
impl Updateable<UpdateRequest> for Entry {
    fn update_with(&self, e: &UpdateRequest) -> Entry {
        Entry {
            ...
            provider: if e.provider.is_some() { e.provider.to_owned().into() } else { self.provider.to_owned() },
            receiver: if e.receiver.is_some() { e.receiver.to_owned().into() } else { self.receiver.to_owned() },
            input_of: if e.input_of.is_some() { e.input_of.to_owned().into() } else { self.input_of.to_owned() },
            output_of: if e.output_of.is_some() { e.output_of.to_owned().into() } else { self.output_of.to_owned() },
            resource_inventoried_as: if e.resource_inventoried_as.is_some() { e.resource_inventoried_as.to_owned().into() } else { self.resource_inventoried_as.to_owned() },
            resource_classified_as: if e.resource_classified_as.is_some() { e.resource_classified_as.to_owned().into() } else { self.resource_classified_as.to_owned() },
            resource_conforms_to: if e.resource_conforms_to.is_some() { e.resource_conforms_to.to_owned().into() } else { self.resource_conforms_to.to_owned() },
            resource_quantity: if e.resource_quantity.is_some() { e.resource_quantity.to_owned().into() } else { self.resource_quantity.to_owned() },
            effort_quantity: if e.effort_quantity.is_some() { e.effort_quantity.to_owned().into() } else { self.effort_quantity.to_owned() },
            available_quantity: if e.available_quantity.is_some() { e.available_quantity.to_owned().into() } else { self.available_quantity.to_owned() },
            has_beginning: if e.has_beginning.is_some() { e.has_beginning.to_owned().into() } else { self.has_beginning.to_owned() },
            has_end: if e.has_end.is_some() { e.has_end.to_owned().into() } else { self.has_end.to_owned() },
            has_point_in_time: if e.has_point_in_time.is_some() { e.has_point_in_time.to_owned().into() } else { self.has_point_in_time.to_owned() },
            due: if e.due.is_some() { e.due.to_owned().into() } else { self.due.to_owned() },
            at_location: if e.at_location.is_some() { e.at_location.to_owned().into() } else { self.at_location.to_owned() },
            agreed_in: if e.agreed_in.is_some() { e.agreed_in.to_owned().into() } else { self.agreed_in.to_owned() },
            finished: if e.finished.is_some() { e.finished.to_owned().unwrap() } else { self.finished.to_owned() },
            in_scope_of: if e.in_scope_of.is_some() { e.in_scope_of.to_owned().into() } else { self.in_scope_of.to_owned() },
            image: if e.image.is_some() { e.image.to_owned().into() } else { self.image.to_owned() },
