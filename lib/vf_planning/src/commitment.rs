...
impl Updateable<UpdateRequest> for Entry {
    fn update_with(&self, e: &UpdateRequest) -> Entry {
        Entry {
            ...
            plan: if e.plan.is_some() { e.plan.to_owned().into() } else { self.plan.to_owned() },
            agreed_in: if e.agreed_in.is_some() { e.agreed_in.to_owned().into() } else { self.agreed_in.to_owned() },
            clause_of: if e.clause_of.is_some() { e.clause_of.to_owned().into() } else { self.clause_of.to_owned() },
            independent_demand_of: if e.independent_demand_of.is_some() { e.independent_demand_of.to_owned().into() } else { self.independent_demand_of.to_owned() },
