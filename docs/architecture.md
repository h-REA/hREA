...
The integrity zome defines the following entry types. Rust structs are prefixed `Rea`.
...
| ProcessSpecification | `ReaProcessSpecification` | `rea_process_specification.rs` | name, default units |
...
| ResourceSpecification | `ReaResourceSpecification` | `rea_resource_specification.rs` | name, default units |
...
| Proposal | `ReaProposal` | `rea_proposal.rs` | publishes, reciprocal, proposed_to, unit_based |
...
| RecipeFlow | `ReaRecipeFlow` | `rea_recipe_flow.rs` | rea_action, resource/effort quantities, recipe_input_of, recipe_output_of |
...
- `get_latest_rea_<type>(revision_id) -> Option<Record>` (walks the update chain)
- `get_original_rea_<type>(revision_id) -> Option<Record>`
- `update_rea_<type>({ revision_id, entry }) -> Record` (merges fields, extends the chain)
- `delete_rea_<type>(revision_id) -> ActionHash`
- batch helpers such as `get_rea_<type>s_from_action_hashes(...)`
