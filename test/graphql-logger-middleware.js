...
const injectLoggingCode = (options = {}) => (resolvers) => {
  ...
  const logger = options.logger || ((process.env.GRAPHQL_DEBUG && process.env.GRAPHQL_DEBUG !== '0') ? console.log.bind(console) : () => {})
  const errorLogger = options.errorLogger || ((process.env.GRAPHQL_DEBUG && process.env.GRAPHQL_DEBUG !== '0') ? console.warn.bind(console, '\x1b[1m\x1b[31mGraphQL query error\x1b[0m') : () => {})
