...
runner.registerScenario('fields with default values set are stored on creation', async (s, t, { planning }) => {
  ...
  const createResponse = await planning.call('main', 'create_commitment', { commitment })
  ...
  const readResponse = await planning.call('main', 'get_commitment', { address: createResponse.Ok.commitment.id })
