...
test('links can be written and read between DNAs', async (t) => {
  ...
  try {
    const { cells: [combined], graphQL } = alice
    ...
    const commitmentResponse = await combined.call('commitment', 'create_commitment', { commitment })
    ...
    const eventResp = await combined.call('economic_event', 'create_economic_event', { event })
    ...
    const fulfillmentResp = await combined.call('fulfillment', 'create_fulfillment', { fulfillment })
    ...
    let readResponse = await combined.call('fulfillment', 'get_fulfillment', { address: fulfillmentId })
    ...
    // ASSERT: check event
    readResponse = await combined.call('economic_event', 'get_economic_event', { address: eventId })
    ...
    readResponse = await combined.call('commitment', 'get_commitment', { address: commitmentId })
    ...
    readResponse = await combined.call('fulfillment', 'get_fulfillment', { address: fulfillmentIdObs })
    ...
    const fulfillmentResp2 = await combined.call('fulfillment', 'create_fulfillment', { fulfillment: fulfillment2 })
    ...
    readResponse = await combined.call('economic_event_index', 'query_economic_events', { params: { fulfills: fulfillmentIdObs } })
    ...
    readResponse = await combined.call('commitment_index', 'query_commitments', { params: { fulfilledBy: fulfillmentId } })
