...
runner.registerScenario('satisfactions can be written and read between DNAs by all parties requiring access', async (s, t, { planning, observation }) => {
  ...
  readResponse = await planning.call('satisfaction', 'query_satisfactions', { satisfies: intentId })
  ...
  readResponse = await observation.call('satisfaction', 'query_satisfactions', { satisfied_by: eventId })
  ...
  readResponse = await planning.call('intent', 'query_intents', { satisfied_by: satisfactionId })
  ...
  readResponse = await observation.call('satisfaction', 'query_satisfactions', { satisfied_by: commitmentId })
  t.equal(readResponse.Ok.length, 1, 'read satisfactions by commitment OK')
  t.equal(readResponse.Ok[0].satisfaction.id, satisfactionId, 'Satisfaction.satisfiedBy indexed correctly')

  // ASSERT: check intent satisfaction query indexes
  readResponse = await planning.call('intent', 'query_intents', { satisfied_by: satisfactionId2 })
