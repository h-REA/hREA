...
export default (dnaConfig: DNAIdMappings, conductorUri: string) => {
  const readOne = mapZomeFn<ReadParams, ProcessSpecificationResponse>(dnaConfig, conductorUri, 'hrea_combined', 'process_specification', 'get_process_specification')
  const readAll = mapZomeFn<PagingParams, ProcessSpecificationConnection>(dnaConfig, conductorUri, 'hrea_combined', 'process_specification_index', 'read_all_process_specifications')
