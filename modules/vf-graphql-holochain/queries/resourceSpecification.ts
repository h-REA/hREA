...
export default (dnaConfig: DNAIdMappings, conductorUri: string) => {
  const readOne = mapZomeFn<ReadParams, ResourceSpecificationResponse>(dnaConfig, conductorUri, 'hrea_combined', 'resource_specification', 'get_resource_specification')
  const readAll = mapZomeFn<PagingParams, ResourceSpecificationConnection>(dnaConfig, conductorUri, 'hrea_combined', 'resource_specification_index', 'read_all_resource_specifications')
