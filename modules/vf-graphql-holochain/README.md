...
The `dnaConfig` option allows the callee to specify custom DNA identifiers to bind GraphQL functions to. For each hREA module ID (see the directory names under `/dna_bundles` in this repository), a runtime `CellId` must be provided as an instance of that DNA to bind to.
