...
pub mod builtins;
pub use builtins::{get_all_builtin_actions, get_builtin_action};
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(match s.as_str() {
            ...
            &_ => Err(D::Error::custom(format!(
                "Invalid value for ActionEffect: {}",
                s.as_str()
            )))?,
        })
    }
 ...
 */
pub fn validate_flow_action(
    action_id: String,
    input_process: Option<ActionHash>,
    output_process: Option<ActionHash>,
) -> Result<(), String> {
    if let Some(action) = get_builtin_action(action_id.as_ref()) {
        match action.input_output {
            ProcessType::NotApplicable => {
                if input_process.is_some() || output_process.is_some() {
                    Err(format!(
                        "EconomicEvent of '{:}' action cannot link to processes",
                        action.id
                    )
                    .into())
                } else {
            ...
            ProcessType::Input => {
                if input_process.is_none() {
                    Err(format!(
                        "EconomicEvent input process required for '{:}' action",
                        action.id
                    )
                    .into())
                } else {
            ...
            ProcessType::Output => {
                if output_process.is_none() {
                    Err(format!(
                        "EconomicEvent output process required for '{:}' action",
                        action.id
                    )
                    .into())
                } else {
...
pub fn validate_move_inventories(
    resouce_inventoried_as: Option<ActionHash>,
