...
pub fn create_rea_process_specification(
    ...
    let rea_process_specification_hash = create_entry(&EntryTypes::ReaProcessSpecification(
    ...
    .ok_or(wasm_error!(WasmErrorInner::Guest(
        "Could not find the newly created ReaProcessSpecification".to_string()
    )))?;
    let tag_prefix = LinkTag(rea_process_specification_hash.get_raw_39().to_vec());
...
#[hdk_extern]
pub fn get_latest_rea_process_specification(
    ...
    let latest_rea_process_specification_hash = match latest_link {
        Some(link) => {
            link.target
                .clone()
                .into_action_hash()
        ...
        None => original_rea_process_specification_hash.clone(),
    };
    get(latest_rea_process_specification_hash, GetOptions::default())
}

#[hdk_extern]
pub fn get_original_rea_process_specification(
    original_rea_process_specification_hash: ActionHash,
) -> ExternResult<Option<Record>> {
    let Some(details) = get_details(
        original_rea_process_specification_hash,
        GetOptions::default(),
    )?
    else {
        return Ok(None);
...
#[hdk_extern]
pub fn get_all_revisions_for_rea_process_specification(
    original_rea_process_specification_hash: ActionHash,
) -> ExternResult<Vec<Record>> {
    let Some(original_record) =
        get_original_rea_process_specification(original_rea_process_specification_hash.clone())?
    else {
        return Ok(vec![]);
    };
    let links = get_links(
...
/// All-`Option` mirror of `ReaProcessSpecification`, so a partial update payload
/// deserializes at the extern boundary. `ReaProcessSpecification` has required fields,
/// which made a sparse update fail before it ever reached the merge.
/// Mirrors the `ReaUnitUpdateParams` pattern.
#[derive(Serialize, Deserialize, Debug)]
pub struct ReaProcessSpecificationUpdateParams {
...
#[derive(Serialize, Deserialize, Debug)]
pub struct UpdateReaProcessSpecificationInput {
    pub revision_id: ActionHash,
    pub entry: ReaProcessSpecificationUpdateParams,
}
...
#[hdk_extern]
pub fn get_all_deletes_for_rea_process_specification(
    original_rea_process_specification_hash: ActionHash,
...
#[hdk_extern]
pub fn get_oldest_delete_for_rea_process_specification(
    original_rea_process_specification_hash: ActionHash,
) -> ExternResult<Option<SignedActionHashed>> {
    let Some(mut deletes) =
        get_all_deletes_for_rea_process_specification(original_rea_process_specification_hash)?
    else {
        return Ok(None);
    };
