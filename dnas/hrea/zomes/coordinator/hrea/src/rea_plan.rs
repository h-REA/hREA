...
pub fn create_rea_plan(rea_plan: ReaPlan) -> ExternResult<Record> {
    let rea_plan_hash = create_entry(&EntryTypes::ReaPlan(rea_plan.clone()))?;
    let record = get(rea_plan_hash.clone(), GetOptions::default())?.ok_or(wasm_error!(
        WasmErrorInner::Guest("Could not find the newly created ReaPlan".to_string())
    ))?;
    let path = Path::from("all_plans");
    let tag_prefix = LinkTag(rea_plan_hash.get_raw_39().to_vec()); // Convert ActionHash to Vec<u8>
    create_link(
...
pub fn get_latest_rea_plan(revision_id: ActionHash) -> ExternResult<Option<Record>> {
    ...
    let latest_rea_plan_hash = match latest_link {
        Some(link) => {
            link.target
                .clone()
                .into_action_hash()
    ...
    get(latest_rea_plan_hash, GetOptions::default())
}

#[hdk_extern]
pub fn get_original_rea_plan(revision_id: ActionHash) -> ExternResult<Option<Record>> {
...
#[hdk_extern]
pub fn delete_rea_plan(revision_id: ActionHash) -> ExternResult<ActionHash> {
    // get latest revision
    ...
    let path = AnyLinkableHash::from(Path::from("all_plans").path_entry_hash()?);
    delete_links(path, id.clone().into(), LinkTypes::AllPlans)?;
    delete_entry(id)
}

#[hdk_extern]
pub fn get_all_deletes_for_rea_plan(
...
#[hdk_extern]
pub fn get_oldest_delete_for_rea_plan(
    revision_id: ActionHash,
) -> ExternResult<Option<SignedActionHashed>> {
    let Some(mut deletes) = get_all_deletes_for_rea_plan(revision_id)? else {
        return Ok(None);
    };
