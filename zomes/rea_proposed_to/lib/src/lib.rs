/**
* hREA proposal addresses zome library API
*
...
pub fn handle_create_proposed_to<S>(entry_type_id: S, proposed_to: CreateRequest) -> RecordAPIResult<ResponseData>
    where S: AsRef<str> + std::fmt::Display,
{
    let (meta, base_address, entry_resp): (_, ProposedToAddress, EntryData) = create_record::<EntryDefinitions,_,_,_,_,_,_,_,_>(read_index_zome, &entry_type_id, proposed_to.to_owned())?;
...
pub fn handle_get_proposed_to(address: ProposedToAddress) -> RecordAPIResult<ResponseData>
{
...
fn construct_response<'a>(address: &ProposedToAddress, meta: &SignedActionHashed, e: &EntryData) -> RecordAPIResult<ResponseData> {
    Ok(ResponseData {
