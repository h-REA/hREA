...
pub fn receive_query_satisfactions<S>(intent_entry_def_id: S, commitment_entry_def_id: S, params: QueryParams) -> RecordAPIResult<Vec<ResponseData>>
    where S: AsRef<str>
{
    handle_query_satisfactions(intent_entry_def_id, commitment_entry_def_id, &params)
}

fn is_satisfiedby_commitment(event_or_commitment: &EventOrCommitmentAddress) -> OtherCellResult<CommitmentResponse> {
    call_local_zome_method(
        |conf: DnaConfigSlice| { conf.satisfaction.commitment_zome },
...
fn handle_query_satisfactions<S>(intent_entry_def_id: S, commitment_entry_def_id: S, params: &QueryParams) -> RecordAPIResult<Vec<ResponseData>>
    where S: AsRef<str>,
{
