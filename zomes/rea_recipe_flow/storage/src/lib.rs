...
impl TryFrom<CreateRequest> for EntryData {
    ...
    fn try_from(e: CreateRequest) -> RecordAPIResult<EntryData> {
        Ok(EntryData {
            ...
            provider_role: e.provider_role.to_owned().into(),
            receiver_role: e.receiver_role.to_owned().into(),
            instructions: e.instructions.to_owned().into(),
            ...
            recipe_reciprocal_clause_of: e.recipe_reciprocal_clause_of.to_owned().into(),
...
impl Updateable<UpdateRequest> for EntryData {
    fn update_with(&self, e: UpdateRequest) -> RecordAPIResult<EntryData> {
        Ok(EntryData {
            ...
            provider_role: if e.provider_role== MaybeUndefined::Undefined { self.provider_role.to_owned() } else { e.provider_role.to_owned().into() },
            receiver_role: if e.receiver_role== MaybeUndefined::Undefined { self.receiver_role.to_owned() } else { e.receiver_role.to_owned().into() },
            instructions: if e.instructions== MaybeUndefined::Undefined { self.instructions.to_owned() } else { e.instructions.to_owned().into() },
            ...
            recipe_reciprocal_clause_of: if e.recipe_reciprocal_clause_of == MaybeUndefined::Undefined { self.recipe_reciprocal_clause_of.to_owned() } else { e.recipe_reciprocal_clause_of.to_owned().into() },
