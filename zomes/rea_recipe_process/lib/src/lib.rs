...
{
    let (meta, base_address, entry_resp): (_,_, EntryData) = create_record::<EntryDefinitions,_,_,_,_,_,_,_,_>(read_index_zome, &entry_def_id, recipe_process.to_owned())?;
...
pub fn construct_response<'a>(
    ...
    Ok(ResponseData {
        recipe_process: Response {
            ...
            recipe_inputs: recipe_inputs.to_owned(),
            recipe_outputs: recipe_outputs.to_owned(),
        }
    })
}
...
pub fn get_link_fields(recipe_process: &RecipeProcessAddress) -> RecordAPIResult<(
    ...
    Ok((
        read_index!(recipe_process(recipe_process).recipe_inputs)?,
        read_index!(recipe_process(recipe_process).recipe_outputs)?,
    ))
}
