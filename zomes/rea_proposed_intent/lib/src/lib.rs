...
use hc_zome_rea_proposed_intent_integrity::*;
...
{
    let (meta, base_address, entry_resp): (_, ProposedIntentAddress, EntryData) = create_record(read_index_zome, &entry_def_id, proposed_intent.to_owned())?;
