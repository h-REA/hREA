/**
 * hREA commitment zome library API
 *
...
{
    let (meta, base_address, entry_resp): (_,_, EntryData) = create_record::<EntryDefinitions,_,_,_,_,_,_,_,_>(read_index_zome, &entry_type_id, commitment.to_owned())?;
