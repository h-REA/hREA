/**
 * hREA resource specification zome library API
 *
...
pub fn handle_create_resource_specification<S>(entry_type_id: S, resource_specification: CreateRequest) -> RecordAPIResult<ResponseData>
    where S: AsRef<str> + std::fmt::Display,
{
    let (meta, base_address, entry_resp): (_,_, EntryData) = create_record::<EntryDefinitions,_,_,_,_,_,_,_,_>(read_index_zome, &entry_type_id, resource_specification)?;
